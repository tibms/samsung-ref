...
#if defined(CONFIG_BQ2429X_NOTIFIER)
...
#endif
#if defined(CONFIG_BQ2429X_NOTIFIER)
struct	notifier_block bq2429x_usb_nb;
#endif
#if defined(CONFIG_VBUS_NOTIFIER)
...
#if defined (CONFIG_BQ2429X_NOTIFIER)
static int bq2429x_usb_handle_notification(struct notifier_block *nb,
		unsigned long action, void *data)
{

	bq2429x_attached_dev_t attached_dev = *(bq2429x_attached_dev_t *)data;

	struct otg_notify *o_notify = get_otg_notify();

	switch (attached_dev) {
	case ATTACHED_DEV_USB_DCP_BQ2429X:
	case ATTACHED_DEV_USB_SDP_BQ2429X:
		if (action == BQ2429X_NOTIFY_CMD_DETACH)
			send_otg_notify(o_notify, NOTIFY_EVENT_VBUS, 0);
		else if (action == BQ2429X_NOTIFY_CMD_ATTACH)
			send_otg_notify(o_notify, NOTIFY_EVENT_VBUS, 1);
		else
			pr_err("%s - ACTION Error!\n", __func__);
		break;
...
#ifdef CONFIG_BQ2429X_NOTIFIER
	bq2429x_notifier_register(&pdata->bq2429x_usb_nb, bq2429x_usb_handle_notification,
...
#ifdef CONFIG_BQ2429X_NOTIFIER
	bq2429x_notifier_unregister(&pdata->bq2429x_usb_nb);
#endif
	return 0;
}
